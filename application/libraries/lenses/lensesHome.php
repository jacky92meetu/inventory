...
class lensesHome extends lensesMain{
    ...
    function get_data(){
        ...
        if(($result = $this->CI->db->query('select c.name store_name, a.payment_date
            ,round(sum(
                    (ifnull(a.selling_price,0) * ifnull(a.quantity,0) / ifnull(er.rate,1))
                + (ifnull(a.shipping_charges_received,0) / ifnull(er.rate,1))
                    - (ifnull(a.shipping_charges_paid,0) / ifnull(er.rate,1))
                    - (ifnull(a.sales_fees_pect,0) / 100 * ifnull(a.selling_price,0) * ifnull(a.quantity,0) / ifnull(er.rate,1))
